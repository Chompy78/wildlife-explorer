...
describe('CamperScreen',()=>{
  ...
  it.each(['mountains','lake','safari','rainforest','alien-planet'] as const)('offers no travel action for the %s preview even when pinned',(destinationId)=>{const save={...createDefaultSave(),wildCamperUnlocked:true,camperIntroductionSeen:true,selectedDestination:destinationId};render(<CamperScreen saveData={save} onSaveChange={vi.fn()} onReturnToPark={vi.fn()} onGoHome={vi.fn()}/>);expect(screen.getByText(/pinned to route map/i)).toBeInTheDocument();expect(screen.queryByRole('button',{name:/^travel to/i})).not.toBeInTheDocument();});
});
