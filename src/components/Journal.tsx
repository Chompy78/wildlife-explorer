...
function JournalEntry({ animal, saveData }: { animal:Animal; saveData:SaveData }) {
  ...
  return <article className={discovered ? 'journal-entry' : 'journal-entry undiscovered'}><div className="animal-emoji" aria-hidden="true">{discovered ? animal.emoji : locked ? '\uD83D\uDD12' : '\u2B1C'}</div><div><h3>{discovered ? animal.name : locked ? `${animal.name} - later` : animal.name}</h3>{helped ? <><p><strong>Status:</strong> Helped and reunited with owner.</p><p>{animal.funFact}</p></> : null}{!helped&&photographed ? <><p><strong>Habitat:</strong> {animal.habitat}</p><p>{animal.funFact}</p></> : null}{!helped&&!photographed&&owlSpotted ? <p className="muted">Spotted at the Strange Old Tree. Use the camera to photograph the owl.</p> : null}{!discovered ? <p className="muted">{locked ? 'This entry belongs to a later milestone or quest.' : `Not photographed yet. Explore ${animal.habitat}.`}</p> : null}</div></article>;
}
